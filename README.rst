...
To run h2load against HTTP/3 server, specify h3-27 ALPN with
``--npn-list`` option like so:

.. code-block:: text

    $ h2load --npn-list h3-27 https://127.0.0.1:4433
...
.. code-block:: text
    ...
    $ docker run --rm -it --network=host nghttp2-quic /usr/local/bin/h2load --npn-list h3-27 https://127.0.0.1:4433
